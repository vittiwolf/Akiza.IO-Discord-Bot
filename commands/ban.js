...
module.exports = {
    name: "ban",
    description: "Ban a user from the Akiza.IO panel",
    ...
    try {
    await fetch(`https://akiza.io/handler/api.php?program_key=${programkey}&api_key=${apikey}&action=ban&user_to=${user}`)
    .then(res => res.json())
    .then(message.channel.send(new Discord.MessageEmbed().setTitle('User Successfully Banned!').addField('Banned By:', message.author).addField('User Banned:', `\`${args[0]}\``).setColor("YELLOW").setTimestamp()));
    }catch(error) {
